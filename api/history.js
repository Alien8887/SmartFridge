...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    /* ── POST: store a batch of readings ─────────────────── */
    if (req.method === 'POST') {
      const { type, points } = req.body || {};
      if (!type || !Array.isArray(points) || points.length === 0)
        return res.status(400).json({ error: 'Missing type or points' });
    ...
    if (req.method === 'GET') {
      ...
      for (const date of dates) {
        const raw = await redis.get(`hist:${type}:${username}:${date}`);
        if (Array.isArray(raw)) allPts.push(...raw);
      }
      ...
      if (filtered.length > 300) {
        const step = Math.floor(filtered.length / 300);

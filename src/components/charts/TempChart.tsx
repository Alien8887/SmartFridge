...
interface TempChartProps { data: AggregatedPoint[]; darkMode: boolean; rangeMs?: number; }
...
export function TempChart({ data, darkMode, rangeMs = 3_600_000 }: TempChartProps) {
  ...
  const forecastCurve = realTs && showForecast ? computeForecastCurve(data, forecastAheadMs, TEMP_BOUNDS) : [];
      ...
      <ResponsiveContainer width="100%" height={250}>
        <ComposedChart data={combined} margin={{ top: 10, right: 20, left: 0, bottom: 0 }}>
          ...
          <Tooltip contentStyle={{ backgroundColor: darkMode ? '#1e293b' : '#fff', border: `1px solid ${darkMode ? '#475569' : '#e2e8f0'}`, borderRadius: '12px' }}
            formatter={(v: number, key: string) => key === 'forecastValue' ? [`${v?.toFixed(1)}°C`, 'Predicted'] : key === 'ewmaValue' ? [`${v?.toFixed(1)}°C`, 'Trend'] : [`${v?.toFixed(1)}°C`, 'Temperature']}
            labelFormatter={realTs ? (ts: number) => new Date(ts).toLocaleString() : undefined} />
          <Area type="monotone" dataKey="value" stroke="#60A5FA" strokeWidth={2} strokeOpacity={isStale ? 0.35 : 1} fillOpacity={isStale ? 0.4 : 1} fill={`url(#${gradientId})`} isAnimationActive={false} connectNulls={false} />

...
function hashPw(pw) {
  return crypto.createHash('sha256').update(pw + (process.env.JWT_SECRET || 'fallback')).digest('hex');
}
...
module.exports = async function handler(req, res) {
  ...
  for (const u of DEFAULT_USERS) {
    ...
    if (!exists) {
      await redis.set(key, { username: u.username, passwordHash: hashPw(u.password), createdAt: Date.now(), role: u.role });

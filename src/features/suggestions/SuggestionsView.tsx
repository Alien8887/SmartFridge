...
import {
  ...
  ShoppingBag, TrendingDown, Star, Heart, Download,
} from 'lucide-react';
...
function buildMatches(inv: InventoryItem[], ratings: Record<string, number>): RecipeMatch[] {
  return recipes.map(recipe => {
    ...
    recipe.ingredients.forEach(ing => {
      ...
      if (found) {
        ...
        const d = getDaysUntilExpiry(found.expiry, found.addedDate);
    ...
    return { recipe, canMake: missing.length === 0, matchScore, matchedItems, missingIngredients: missing, urgencyLevel, minDays: minDays === 999 ? 99 : minDays };
  }).sort((a, b) => {
    ...
    return (ratings[b.recipe.id] ?? 3) - (ratings[a.recipe.id] ?? 3);
  });
}
...
function RecipeCard({ match, ratings, onRate, darkMode, theme, onSelect }: {
  match: RecipeMatch; ratings: Record<string, number>; onRate: (id: string, stars: number) => void; darkMode: boolean; theme: Theme; onSelect: () => void;
}) {
    ...
    <div className={`animate-fade-in card-hover ${theme.card} border rounded-xl p-4 ${border}`}>
      ...
      <div className="flex items-center justify-between mt-3">
        ...
        <div className="flex flex-wrap gap-1">
          {r.tags.slice(0, 2).map(tag => <span key={tag} className={`text-xs px-2 py-0.5 rounded-full ${darkMode ? 'bg-slate-700 text-slate-400' : 'bg-slate-100 text-slate-600'}`}>{tag}</span>)}
        </div>
      </div>
    </div>
  );
...
function RecipeModal({ match, onClose, onCook, darkMode, theme }: {
  match: RecipeMatch; onClose: () => void; onCook: (match: RecipeMatch) => void; darkMode: boolean; theme: Theme;
}) {
    ...
    <Modal isOpen title={`${r.emoji} ${r.name}`} onClose={onClose} theme={theme}>
      <div className="animate-scale-in space-y-5">
        ...
        <div>
          ...
          <div className="space-y-1">
            {r.ingredients.map(ing => {
              const have = matchedItems.some(i => nameMatch(i.name, ing));
              return (
                <div key={ing} className={`flex items-center gap-2 text-sm ${have ? theme.text : theme.textMuted}`}>
                  {have ? <CheckCircle2 className="w-4 h-4 text-emerald-400 flex-shrink-0" /> : <ShoppingCart className="w-4 h-4 text-red-400 flex-shrink-0" />}
                  <span>{ing}</span>
                  {!have && <span className="text-xs text-red-400 ml-auto">Missing</span>}
                </div>
              );
            })}
          </div>
        ...
        {missingIngredients.length === 0 ? (
          ...
          <p className={`text-xs ${theme.textMuted}`}>Add the missing ingredients above before cooking, or find them in the Shopping List tab.</p>
        )}
      </div>
    </Modal>
  );
...
export function SuggestionsView({ inventory, onAddProduct, onConsume, ratings, onRate, totalConsumed = 0, totalWasted = 0, darkMode, theme }: SuggestionsViewProps) {
  ...
  const displayed = (categoryFilter === 'All' ? matches : matches.filter(m => m.recipe.category === categoryFilter)).slice(0, 12);
  ...
  const handleCook = (match: RecipeMatch) => {
    match.matchedItems.forEach(item => onConsume(item.id, 1));
    setSelectedMatch(null);
  };

  const handleBuy = (name: string) => {
    const catalogEntry = availableProducts.find(p => p.name.toLowerCase() === name.toLowerCase());
    const product: Product = catalogEntry ? catalogEntry : { name, category: 'Other', defaultExpiry: 7 };
    onAddProduct(product, 1, 'pcs');
  ...
  return (
    <div className="space-y-4 md:space-y-6 animate-fade-in">
        ...
        <div className="animate-slide-down bg-red-500/10 border border-red-500/40 rounded-xl p-4">
          ...
          <div className="flex flex-wrap gap-2">
            {urgent.slice(0, 5).map(m => (
              <button key={m.recipe.id} onClick={() => setSelectedMatch(m)} className="text-sm px-3 py-1 rounded-full bg-red-500/20 text-red-400 hover:bg-red-500/30 transition-colors">{m.recipe.emoji} {m.recipe.name}</button>
            ))}
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          { icon: <TrendingDown className="w-5 h-5 text-emerald-400" />, val: String(totalConsumed), label: 'Items used' },
          { icon: <AlertTriangle className="w-5 h-5 text-red-400" />, val: String(totalWasted), label: 'Items wasted' },
      ...
      <div className={`flex rounded-xl overflow-hidden border ${darkMode ? 'border-slate-700' : 'border-slate-200'}`}>
        {(['recipes', 'shopping'] as const).map(tab => (
          <button key={tab} onClick={() => setActiveTab(tab)} className={`flex-1 py-2 text-sm font-medium transition-colors ${activeTab === tab ? 'bg-sky-500 text-white' : `${theme.hover} ${theme.text}`}`}>
      ...
      {activeTab === 'recipes' && (
        <div className="space-y-4">
          <div className="flex gap-2 flex-wrap">
            {categories.map(cat => (
              <button key={cat} onClick={() => setCategoryFilter(cat)} className={`px-3 py-1 rounded-full text-xs font-medium transition-colors ${categoryFilter === cat ? 'bg-sky-500 text-white' : darkMode ? 'bg-slate-700 text-slate-300 hover:bg-slate-600' : 'bg-slate-100 text-slate-700 hover:bg-slate-200'}`}>{cat}</button>
            ))}
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {displayed.map(match => <RecipeCard key={match.recipe.id} match={match} ratings={ratings} onRate={onRate} darkMode={darkMode} theme={theme} onSelect={() => setSelectedMatch(match)} />)}
          </div>
        </div>
      )}
      ...
      {selectedMatch && <RecipeModal match={selectedMatch} onClose={() => setSelectedMatch(null)} onCook={handleCook} darkMode={darkMode} theme={theme} />}
    </div>
  );
}
...
function q10Spoilage(items, T) {
  ...
  return items.map(item => {
    ...
    return {
      ...
      nominalDaysLeft: nominal, adjustedDaysLeft: +adjusted.toFixed(1),
      spoilageRisk: +risk.toFixed(1), speedup: +rr.toFixed(2),
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    // Real AI-in-the-loop: decide whether to ask the hardware for a colder
    // goal than the user's manual target, and why. sensors.js reads this back
    // on its next GET and only applies it if it's actually colder than target.
    ...
    if (worst && worst.priority === 'urgent' && worst.spoilageRisk > 70) {
      goalRecommendation = {
        shouldOverride: true,
        recommendedTemp: +Math.max(0, T - 2).toFixed(1),
        reason: `${worst.name} at ${worst.spoilageRisk}% spoilage risk — lowering target to slow decay`,
      };
    } else if (safetyScore < 50) {
      goalRecommendation = {
        shouldOverride: true,
        recommendedTemp: +Math.max(0, T - 1).toFixed(1),
        reason: `Food safety score ${safetyScore}/100 — lowering target as a precaution`,
      };
    }

    if (goalRecommendation.shouldOverride) {
      await redis.set(`ai-goal:${username}`, {
        recommendedTemp: goalRecommendation.recommendedTemp,
        reason: goalRecommendation.reason,
        computedAt: Date.now(),
      }, { ex: 1800 });
    } else {
      await redis.del(`ai-goal:${username}`);
    }
    ...
    return res.status(200).json({
      ...
      anomaly: {
        isAnomaly: anomaly.isAnomaly, zScore: anomaly.z, mean: anomaly.mean, stdDev: anomaly.stdDev,
      ...
      goalRecommendation,
      modelInfo: {
        algorithms: ['EWMA (α=0.3)', 'OLS Linear Regression', 'Z-Score (threshold 2.5σ)', 'Q10 Model (Q10=2.0, Tref=4°C)', 'Food Safety Composite Score'],

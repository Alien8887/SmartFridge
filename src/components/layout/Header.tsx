...
export function Header({ username, role, darkMode, setDarkMode, onLogout, mobileMenuOpen, setMobileMenuOpen, isConnected, theme }: HeaderProps) {
  return (
    <header className={`${theme.card} ${theme.border} border-b backdrop-blur-xl sticky top-0 z-50 shadow-lg`} role="banner">
      <div className="max-w-7xl mx-auto px-4 md:px-6 py-3 md:py-4 flex items-center justify-between">
        <div className="flex items-center gap-2 md:gap-3 min-w-0">
          <button onClick={() => setMobileMenuOpen(!mobileMenuOpen)} className={`lg:hidden p-2 rounded-xl transition-colors ${theme.hover}`} aria-label={mobileMenuOpen ? 'Close navigation menu' : 'Open navigation menu'} aria-expanded={mobileMenuOpen} type="button">
            ...
            <p className={`text-xs ${theme.textMuted} hidden sm:flex items-center gap-1.5 truncate`}>
              ...
              {role && (
                <span className={`px-1.5 py-0.5 rounded-full text-[10px] font-medium capitalize ${
                  role === 'admin' ? 'bg-purple-500/20 text-purple-400' : role === 'guest' ? 'bg-slate-500/20 text-slate-400' : 'bg-sky-500/20 text-sky-400'
                }`}>
        ...
        <div className="flex items-center gap-1 md:gap-2 flex-shrink-0">
          <div className={`hidden sm:flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium ${isConnected ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400' : 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400'}`} title={isConnected ? 'Fridge connected' : 'Fridge offline'}>
          ...
          <div className={`sm:hidden w-2.5 h-2.5 rounded-full ${isConnected ? 'bg-emerald-400' : 'bg-red-400'}`} aria-label={isConnected ? 'Connected' : 'Offline'} />

...
import { UserCircle, ShieldCheck, KeyRound, Trash2, CheckCircle2, Calendar, Loader2 } from 'lucide-react';
...
interface ProfileData { username: string; role: string; createdAt: number; hasDevice: boolean; totalConsumed: number; totalWasted: number; }
...
export function ProfileView({ token, username, darkMode, theme }: ProfileViewProps) {
  ...
  const fetchProfile = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const r = await fetch(`${BASE}/api/auth/profile`, { headers: { Authorization: `Bearer ${token}` } });
    ...
    <div className="space-y-4 md:space-y-6 animate-fade-in">
        ...
        <div>
          ...
          <div className="flex items-center gap-2 mt-1">
            <span className={`text-xs px-2 py-0.5 rounded-full font-medium capitalize ${
              profile?.role === 'admin' ? 'bg-purple-500/20 text-purple-400' : profile?.role === 'guest' ? 'bg-slate-500/20 text-slate-400' : 'bg-sky-500/20 text-sky-400'
            }`}>
      ...
      <div className="grid grid-cols-2 gap-4">
        <Card className={`${theme.card} text-center`}>
          ...
          <div className={`text-2xl font-bold ${theme.text}`}>{profile?.totalConsumed ?? 0}</div>
          <div className={`text-xs ${theme.textMuted}`}>Items consumed</div>
        </Card>
        <Card className={`${theme.card} text-center`}>
          <Trash2 className="w-7 h-7 text-red-400 mx-auto mb-2" />
          <div className={`text-2xl font-bold ${theme.text}`}>{profile?.totalWasted ?? 0}</div>
          <div className={`text-xs ${theme.textMuted}`}>Items wasted</div>
        </Card>
      </div>
      ...
      <Card className={theme.card}>
        <h3 className={`font-semibold ${theme.text} mb-3 flex items-center gap-2`}><KeyRound className="w-5 h-5 text-sky-400" /> Change password</h3>

...
function formatUptime(seconds: number): string { if (seconds < 60) return `${seconds}s`; const h = Math.floor(seconds / 3600); const m = Math.floor((seconds % 3600) / 60); return h === 0 ? `${m}m` : `${h}h ${m}m`; }
    ...
    <div className="space-y-4 md:space-y-6 animate-fade-in">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <Card className={theme.card}>
          ...
          <div className="text-center mb-2">
            <div className={`text-5xl font-bold ${sensorData.connected ? theme.accent : theme.textMuted} animate-number-pop`}>{sensorData.connected ? `${sensorData.temperature.toFixed(1)}°C` : '—'}</div>
        ...
        <Card className={theme.card}>
          ...
          <div className="text-center mb-2">
            <div className={`text-5xl font-bold ${sensorData.connected ? theme.accent : theme.textMuted} animate-number-pop`}>{sensorData.connected ? `${Math.round(sensorData.humidity)}%` : '—'}</div>
      ...
      <Card className={theme.card}>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {[{ label: 'DHT22 sensor', ok: diagnostics.dhtOK }, { label: 'HX711 scale', ok: diagnostics.hxOK }].map(s => (
            <div key={s.label} className={`rounded-lg p-3 text-center ${darkMode ? 'bg-slate-800/60' : 'bg-slate-50'}`}>
              {s.ok === null ? <HelpCircle className={`w-5 h-5 mx-auto ${theme.textMuted}`} /> : s.ok ? <CheckCircle className="w-5 h-5 mx-auto text-emerald-400" /> : <XCircle className="w-5 h-5 mx-auto text-red-400" />}
              <p className={`text-xs mt-1 ${theme.textMuted}`}>{s.label}</p>
              <p className={`text-xs font-medium ${s.ok === null ? theme.textMuted : s.ok ? 'text-emerald-400' : 'text-red-400'}`}>{s.ok === null ? 'No data' : s.ok ? 'OK' : 'Fault'}</p>
            </div>
          ))}
          <div className={`rounded-lg p-3 text-center ${darkMode ? 'bg-slate-800/60' : 'bg-slate-50'}`}>
            ...
            <p className={`text-xs font-medium ${diagnostics.rssi !== null ? rssiInfo(diagnostics.rssi).color : theme.textMuted}`}>{diagnostics.rssi !== null ? `${rssiInfo(diagnostics.rssi).label} (${diagnostics.rssi} dBm)` : 'No data'}</p>
          </div>
          <div className={`rounded-lg p-3 text-center ${darkMode ? 'bg-slate-800/60' : 'bg-slate-50'}`}>
      ...
      <Card className={theme.card}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className={`text-xs ${doorAlarmActive ? 'text-red-400 font-medium' : theme.textMuted}`}>{doorAlarmActive ? 'Buzzer sounding — door open over 30s' : sensorData.doorOpen ? 'Door open, buzzer armed' : 'Door closed, buzzer idle'}</span>
          </div>
        </div>
        <p className={`text-xs ${theme.textMuted} mt-3 flex items-start gap-1.5`}><Info className="w-3.5 h-3.5 flex-shrink-0 mt-0.5" />Servo angle = <code className="font-mono">90 + (actual − goal) × 15</code>, clamped 0–180°.</p>
      </Card>

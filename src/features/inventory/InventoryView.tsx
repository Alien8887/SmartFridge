...
import { Package, Clock, PlusCircle, Trash2, CheckCircle } from 'lucide-react';
    ...
    <div className="space-y-4 md:space-y-6">
      <Card className={theme.card}>
        <div className="flex items-center justify-between mb-4 md:mb-6">
          ...
          <Button onClick={() => setShowAddProduct(true)} variant="primary" icon={<PlusCircle className="w-4 h-4" />}>
                ...
                <div className="flex items-center justify-between gap-3">
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    <div className="text-right mr-2 hidden sm:block">
                      <div className={`text-xs md:text-sm font-bold ${getFreshnessColor(item.freshness, darkMode)}`}>{item.freshness}%</div>
                      <div className={`text-xs ${theme.textMuted} flex items-center gap-1 justify-end mt-1`}>
                        <Clock className="w-3 h-3" /> {item.expiry}d
                      </div>
                    </div>

                    <button onClick={() => onConsume(item.id, item.category)} className="p-2 rounded-lg bg-emerald-500/20 hover:bg-emerald-500/30 transition-colors" title="Mark as Consumed">
                      <CheckCircle className="w-5 h-5 text-emerald-500" />
                    </button>
                    <button onClick={() => onWaste(item.id, item.category)} className="p-2 rounded-lg bg-red-500/20 hover:bg-red-500/30 transition-colors" title="Mark as Waste (Delete)">
                      <Trash2 className="w-5 h-5 text-red-500" />
                    </button>
                  </div>
                </div>
                <div className="mt-3 h-2 bg-slate-700 rounded-full overflow-hidden">
                  <div className={`h-full ${warning.color} transition-all duration-500`} style={{ width: `${item.freshness}%` }} />
                </div>
              </div>
            );
          })}
      ...
      <ProductModal isOpen={showAddProduct} onClose={() => setShowAddProduct(false)} onAddProduct={(p) => { onAddProduct(p); setShowAddProduct(false); }} theme={theme} />
    </div>
  );
}

...
export function DeviceConnectCard({ token, darkMode, theme }: DeviceConnectCardProps) {
  ...
  const fetchDeviceToken = async () => {
    ...
    try {
      ...
      setError('Could not fetch your device code — try again.');
    } finally {
      setLoading(false);
    }
  };
      ...
      {!deviceToken ? (
        <>
          <p className={`text-sm ${theme.textMuted} mb-3`}>
            Generate a private connection code and paste it into your ESP32 sketch's
            <code className="mx-1 px-1.5 py-0.5 rounded bg-purple-500/15 text-purple-400 font-mono text-xs">DEVICE_TOKEN</code>
            constant. Every reading that fridge sends will then belong only to your account.
          </p>
          <button
        ...
        <div className="animate-scale-in space-y-2">
          <div className={`flex items-center gap-2 p-3 rounded-lg font-mono text-xs break-all ${
            darkMode ? 'bg-slate-900 text-slate-200' : 'bg-slate-100 text-slate-800'
          }`}>
            <span className="flex-1">{deviceToken}</span>
            <button
              onClick={copy}
              className="flex-shrink-0 p-1.5 rounded-md bg-slate-700 hover:bg-slate-600 text-white transition-colors"
          ...
          <p className={`text-xs ${theme.textMuted}`}>
            This code does not expire. Anyone with it can post sensor data to your account, so
            treat it like a password and only paste it into your own sketch.
          </p>
        </div>
      )}
    </div>
